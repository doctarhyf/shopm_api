...
//SHOPM
function RepportTable($title, $date, $totItems, $totCash, $header, $data, $isMonthly = false, $totVendu)
{
    //repport title
    ...
    $this->Ln();
    $this->Cell(190,7,'Total Vendu(Fc) : ' . number_format($totVendu),0);
    $this->Ln();
    $this->Cell(190,7,'Total Revenu(Fc) : ' . number_format($totCash) ,0);
    $this->Ln();
    
		    
    
    if($isMonthly){
        $this->Cell(190,7,'Dime : ' . number_format($totCash * .1),0);
        $this->Ln();
    }
    $this->Ln();
    // Header
    $cellw = array(54.28,27.14,27.14,27.14,27.14,27.14);
    $i = 0;
    foreach($header as $col){
