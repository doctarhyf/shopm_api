...
if( $pds->execute($data)){
	...
	function getItemDaillySells($y, $m, $d){
		...
		s.sell_item_origin_price as 'PA',
		s.sell_item_cur_price as 'PV',
		(s.sell_item_cur_price - s.sell_item_origin_price) as 'B',
		((s.sell_item_cur_price - s.sell_item_origin_price) * s.sell_qty) as 'B',
...
$pds = $pdo->prepare($sql);
	...
	function getIDFromTableByName($tableName, $val, $rowId, $rowName){
		...
		//shopm
	function updItem($item_id, $item_name,$item_price,$item_stock_count, $item_desc, $item_origin_price){
		...
		item_price='$item_price', item_stock_count='$item_stock_count', item_desc='$item_desc', item_last_stock_upd=CURRENT_TIMESTAMP, item_origin_price=$item_origin_price 
